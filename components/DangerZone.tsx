...
const DangerZone = () => {
   ...
   const handleResetApp=async()=>{
    Alert.alert(
        "Reset App","This will delete all the todos",[{text:"Cancel", style:"cancel"},
        {
          text:"Delete All",
          style:"destructive",
          onPress:async()=>{
            try{
                ...
                Alert.alert("App Reset",`Succesfullty deleted ${result.deletedCount} todo${result.deletedCount===1?"":"s"}.Your app has been reset.`);
            }catch(error){
  ...
  return (
    <LinearGradient colors={colors.gradients.surface} style={settingStyles.section}>
        ...
        <TouchableOpacity style={[settingStyles.actionButton,{borderBottomWidth:0}]} onPress={handleResetApp} activeOpacity={0.7}>
